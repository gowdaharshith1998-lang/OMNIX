...
const HYB_FN_ID =
  "apps/backend/src/axiom/services/crypto/hybrid_signer.py::verify_hybrid";
...
vi.mock("@/lib/api", () => ({
  ...
  getFile: vi.fn(() =>
    Promise.resolve({
      path: "apps/backend/src/axiom/services/crypto/hybrid_signer.py",
  ...
  GraphCanvas: React.forwardRef(function MockGraphCanvas(
    ...
    React.useEffect(() => {
      ...
      if (!spec || spec.kind === "repo") setScene("repo");
        ...
        if (p === "apps/backend/src/axiom") setScene("av2");
        else if (p === "apps/backend/src/axiom/services/crypto") setScene("crypto");
      ...
      <div data-testid="mock-graph">
        ...
        <button
          ...
          onClick={() =>
            props.onViewerScope?.({
              kind: "directory",
              path: "apps/backend/src/axiom/services/crypto",
            })
          }
        >
          crypto
...
describe("slice15.1 atomic drill integration", () => {
  ...
  it("leaf selection: FUNCTION badge + Code tab body non-empty", async () => {
    const { root, container } = await renderWorkspace();
    await act(async () => {
      container
        .querySelector('[data-testid="go-crypto"]')!
    ...
    await act(async () => {
      ...
      const innerCode = [...container.querySelectorAll(".xray-itabs [role='tab']")].find(
        (b) => b.textContent?.trim().toUpperCase() === "CODE"
      );
      expect(innerCode).toBeTruthy();
      innerCode!.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
