...
const FIXTURE_NODES: GraphNode[] = [
  {
    ...
    name: "verify_hybrid",
    type: "function",
    file_path: "apps/backend/src/axiom/services/crypto/hybrid_signer.py",
    line_start: 10,
    line_end: 42,
  },
];
...
describe("slice15 drill-down integration", () => {
  ...
  it("leaf verify_hybrid shows FUNCTION eyebrow and Code tab shows non-empty source", async () => {
    const { root, container } = await renderWorkspace();
    await act(async () => {
      container.querySelector<HTMLButtonElement>("[data-testid=\"go-crypto\"]")!.click();
    });
