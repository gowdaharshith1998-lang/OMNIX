...
# JARs with fixed timestamps, and runs smoke tests to confirm functional
# integrity.
#
...
# ----- Compile harnesses ----------------------------------------------------
echo "→ compiling JavaSemanticEmitter.java + JavaEquivalenceHarness.java"
...
(
    cd "${VENDOR_DIR}"
    sha256sum "${CORE_JAR}" "${EMITTER_JAR}" "${EQUIVALENCE_JAR}" "${SS_JAR}" "${JAVASSIST_JAR}" > SHA256SUMS
...
echo "    artifacts: ${VENDOR_DIR}/${EMITTER_JAR}, ${VENDOR_DIR}/${EQUIVALENCE_JAR}"
