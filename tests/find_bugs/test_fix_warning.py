...
from src.fabric.config import default_config
from src.find_bugs import cli as fbcli
...
def test_fix_warning_emits_when_chain_includes_paid_provider(
    ...
    monkeypatch.setattr("src.fabric.config.load_config", lambda: c)
...
def test_fix_warning_suppressed_when_dry_run_or_ollama_only(
    ...
    monkeypatch.setattr("src.fabric.config.load_config", lambda: c2)
    fbcli._emit_fix_fabric_warning_if_needed(True)
    assert capsys.readouterr().err == ""
