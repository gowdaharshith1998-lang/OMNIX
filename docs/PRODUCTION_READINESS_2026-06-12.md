...
| F29 | High | **Per-finding / per-rebuild receipts were classical Ed25519 only** (F21 had corrected the marketing to match). | Implemented NIST-style **hybrid signing**: each per-finding and per-rebuild receipt now carries an ML-DSA-65 (FIPS 204) signature (`<id>.mldsa.sig`) over the same canonical bytes, alongside the existing Ed25519 `.sig`. The scan verifier requires the PQC signature when present; new `verify_rebuild_mldsa`. So every per-item receipt is now directly post-quantum signed (not only Merkle-anchored under the ML-DSA manifest). The Ed25519 project key is retained for the provider-vault KDF. README claim restored to the accurate strong form. 3 new tests (finding PQC tamper, rebuild PQC round-trip + tamper). |
